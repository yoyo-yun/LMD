...
class UniterEncoder(nn.Module):
    ...
    def forward(self, input_, attention_mask,
                output_all_encoded_layers=True, return_attention=False):
        all_encoder_layers = []
        ...
        for layer_module in self.layer:
            if return_attention:
                hidden_states, attention_output = layer_module(hidden_states, attention_mask, return_attention)
                attention_list.append(attention_output)
            else:
        ...
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
        if return_attention:
            return all_encoder_layers, attention_list
        else:
...
class UniterModel(UniterPreTrainedModel):
    ...
    def _compute_img_txt_embeddings(self, input_ids, position_ids,
                                    ...
                                    txt_type_ids=None, img_type_ids=None, return_all_embeddings=False):
        ...
        if not return_all_embeddings:
            ...
            return  img_emb, txt_emb, embedding_output

    def forward(self, input_ids, position_ids,
                img_feat, img_pos_feat,
                attention_mask, adv_training, adv_modality, 
                adv_delta_txt, adv_delta_img, gather_index=None, img_masks=None,
                output_all_encoded_layers=True,
                txt_type_ids=None, img_type_ids=None, return_embedding = False, return_attention=False):
        ...
        if input_ids is None:
            ...
            img_emb, txt_emb, embedding_output = self._compute_img_txt_embeddings(
                input_ids, position_ids,
                img_feat, img_pos_feat,
                gather_index, adv_training, adv_modality, 
                adv_delta_txt, adv_delta_img, 
                img_masks, txt_type_ids, img_type_ids, return_all_embeddings=True)

        if return_attention:
            encoded_layers, attention_list = self.encoder(
                embedding_output, extended_attention_mask,
                output_all_encoded_layers=output_all_encoded_layers, return_attention=return_attention)
        else:
        ...
        if not return_embedding:
            if return_attention :
                return encoded_layers, attention_list
            else:
                return encoded_layers
        else:
            if return_attention:
                return encoded_layers, img_emb, txt_emb, embedding_output, attention_list
            else:
                return encoded_layers, img_emb, txt_emb, embedding_output
